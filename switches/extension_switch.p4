...
Register<bit<32>, _>(1) device4_status;
Register<bit<32>, _>(1) device5_status;
Register<bit<32>, _>(1) device6_status;
Register<bit<32>, _>(1) device7_status;
Register<bit<32>, _>(1) device8_status;


Register<bit<32>, _>(1) available_device_address;

Register<bit<32>, _>(1) active_policy; //this is a workaround for defining which block we should use. 
...
control SwitchIngress(
    ...
    //-------------------policy definer -------------------------------//
    RegisterAction<bit<32>, _, bit<32>>(active_policy) read_active_policy = {
    ...
    //----------- BEGIN round robin actions -----------------------------//
    //this will just read a queue element 
    ...
    //This will keep a single integer as a iterator for the circular queue. Considering a queue of size 10 for now
    ...
    RegisterAction<bit<32>, _,bit<32>>(device4_status) update_device4_status = {
        void apply(inout bit<32> value, out bit<32> rv) {
            if(hdr.agent.src_addr == 4){
                value = hdr.agent.cpu_util;
            }
            rv = value;
        }
    ...
    RegisterAction<bit<32>, _,bit<32>>(device5_status) update_device5_status = {
        void apply(inout bit<32> value, out bit<32> rv) {
            if(hdr.agent.src_addr == 5){
                value = hdr.agent.cpu_util;
            }
            rv = value;
        }
    ...
    RegisterAction<bit<32>, _,bit<32>>(device6_status) update_device6_status = {
        void apply(inout bit<32> value, out bit<32> rv) {
            if(hdr.agent.src_addr == 6){
                value = hdr.agent.cpu_util;
            }
            rv = value;
        }
    ...
    RegisterAction<bit<32>, _,bit<32>>(device7_status) update_device7_status = {
        void apply(inout bit<32> value, out bit<32> rv) {
            if(hdr.agent.src_addr == 7){
                value = hdr.agent.cpu_util;
            }
            rv = value;
        }
    ...
    RegisterAction<bit<32>, _,bit<32>>(device8_status) update_device8_status = {
        void apply(inout bit<32> value, out bit<32> rv) {
            if(hdr.agent.src_addr == 8){
                value = hdr.agent.cpu_util;
            }
            rv = value;
        }
    ...
    apply {
        if (hdr.agent.isValid()) {
             ...
             dev1 = update_device4_status.execute(0);
             min_tmp = min(dev1, min_tmp);
             if (min_tmp == dev1){
             	  final_addr = 0x0A320111;      //device 4 destination
             }
             dev1 = update_device5_status.execute(0);
             min_tmp = min(dev1, min_tmp);
             if (min_tmp == dev1){
             	  final_addr = 0x0A320111;      //device 5 destination
             }
             /*dev1 = update_device6_status.execute(0);
             min_tmp = min(dev1, min_tmp);
             if (min_tmp == dev1){
             	  final_addr = 0x0A320111;      //device 6 destination
             }
             dev1 = update_device7_status.execute(0);
             min_tmp = min(dev1, min_tmp);
             if (min_tmp == dev1){
             	  final_addr = 0x0A320111;      //device 7 destination
             }
             dev1 = update_device8_status.execute(0);
             min_tmp = min(dev1, min_tmp);
             if (min_tmp == dev1){
             	  final_addr = 0x0A320111;      //device 8 destination
             }*/
	     ig_md.available_server_meta = Update_Device.execute(0);
 	   }
        else {
                ig_md.policy_selector_meta = read_active_policy.execute(0);
                //we use selector=0 for minimum finder
                if(ig_md.policy_selector_meta == 0){
                	read_finder_decision();
                }else{ //we use selector!=0 to scheduler block
